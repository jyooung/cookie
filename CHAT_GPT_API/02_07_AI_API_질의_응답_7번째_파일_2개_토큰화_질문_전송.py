...
# 아래의 코드를 수정하여 T100 변수의 내용을 토큰화하는 형태로 수정하여 인공지능에게 API 질문으로 전송하도록 수정하려고 합니다
# 추가로 B200.txt 파일의 질문내용을 문자열 T200 변수로 저장하고, 이를 토큰화하여 인공지능에게 T100, T200 의 토큰화된 내용을 질문으로 던지고 답변을 모두 받을 수 있도록 하는 코드로 아래의 소스코드를 수정해줄래 ?
...
# T100과 T200의 내용을 토큰화
tokens_T100 = word_tokenize(T100)
tokens_T200 = word_tokenize(T200)

# 토큰화된 내용을 문자열로 변환하여 인공지능에게 전송
content_T100 = ' '.join(tokens_T100)
content_T200 = ' '.join(tokens_T200)

# ChatGPT 모델에 토큰화된 두 개의 질문을 동시에 보내는 코드
response = openai.ChatCompletion.create(
    model="gpt-3.5-turbo",
    messages=[
        {"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": content_T100},  # T100의 토큰화된 내용
        {"role": "user", "content": content_T200}   # T200의 토큰화된 내용
    ]
)
